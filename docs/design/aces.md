...
| `D6` | **황금 칩 확정 (확정, Q11 + 명칭 2026-06-11)**: 기본 조건 6종 판정 — **redjack/bluejack**(무게 정확 일치)·**red guys/blue guys**(만석)·콤보 **closejack**(+1)·**full guys**(+1), **기본 최대 6** — 후 효과 칩(E5) 가산. **버스트 핸드는 red/blue guys 절대 불인정**, **유효 무게 = 한계치면 최대 무게 칩 인정**(버스트-redjack 명칭 통일 = Q22). **칩은 스탠드 시 리셋** | E5 |
...
| Q22 | **버스트-redjack 명칭 통일** — GDD Q11은 버스트라도 유효 무게(3번 슬롯) = 한계치면 최대 무게 칩 인정. 그런데 카드 용어 'Redjack'(`aces_items.md`)은 "**버스트가 아니며** 정확 일치"로 더 엄격. 버스트 케이스가 redjack/bluejack(나아가 closejack) 판정에 포함되는가? | 칩 판정(D6), `EXACT`·`WRECK` 카드 조건 |
