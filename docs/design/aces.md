...
| `F13` | **군 라벨 참조 일체**(자기 군 포함 — 군은 플레이어 비가시 은닉 정보) / 군 MP 예산 초과 | 시너지는 가시적 메커니즘으로만(§3.5) + '정해' 방지 |
...
> 확정된 항목만 §15 스키마 YAML로 이 섹션(또는 분리 파일)에 누적한다. §13 배분·§14 금지 조합·§3-1 CE/MP 예산 준수.
